...
export function ContactForm() {
  ...
  const GOOGLE_FORM_URL = portfolioConfig.contact.formUrl
  const FIELD_IDS = portfolioConfig.contact.fieldIds
      ...
      <CardContent className="space-y-6">
        ...
        <div className="text-center space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <Button variant="outline" asChild>
              <Link href={`tel:${portfolioConfig.social.phone}`}>
                <Phone className="mr-2 h-4 w-4" />
                {portfolioConfig.social.phone.replace('+', '').replace(/(\d{3})(\d{3})(\d{4})/, '+$1 $2-$3')}
              </Link>
            </Button>
          </div>
        </div>
